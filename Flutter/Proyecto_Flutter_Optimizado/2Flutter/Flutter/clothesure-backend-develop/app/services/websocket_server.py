...
class ClothesureSocketIOServer:
    def __init__(self, host: str = "0.0.0.0", port: int = 3000):
        ...
        async def reaction(sid, data):
            # Compatibilidad con Flutter - evento 'reaction'
            action = data.get('action', 'like')
            await self._handle_like_unlike(sid, data, action=action)
    ...
    async def _handle_like_unlike(self, sid, data, action: str):
        try:
            ...
            success_payload = {
                'outfitId': outfit_id,
                'action': action,
                'routingKey': f'reaction.{action}',
                'timestamp': data.get('timestamp'),
                'user_id': user_id
            }
            await self.sio.emit('reaction_success', success_payload, to=sid)
            ...
            await self.emit_event('like_update', broadcast_payload, room=f'outfit_{outfit_id}')
